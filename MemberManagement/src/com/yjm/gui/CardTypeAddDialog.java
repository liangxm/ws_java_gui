 ...
 */
public class CardTypeAddDialog extends AbstractDialog {
	...
	public CardTypeAddDialog(JFrame frame){
		super(frame);
		initComponent();
		this.add(pane);
		...
		btnsersbumit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(txttypename.getText().trim().length()==0){
					...
					if(cardTypeService.addCardType(cardType)){
						JOptionPane.showMessageDialog(null, "新卡类型添加成功！");
						dispose();
					}else{
						JOptionPane.showMessageDialog(null, "新卡类型添加失败！");
					}
				}
			}
		});
